...
async function instintiateConnection(
  func: { (): Promise<void>; (): any },
  retries: number,
...
const fixtures = base.extend<MyFixtures>({
  page: async ({ baseURL }, use) => {
    instintiateConnection(
      async () => {
        await _android.launchServer({ deviceSerialNumber: "d991b35d", omitDriverInstall: false }).then(async (res) => {
  ...
  mainPage: async ({ page }, use) => {
    const abstractPage = new MainPage(page);
    await use(abstractPage);
  },
